defmodule LocacaoApi.Midias.Midia do
  ...
  schema "midia" do
    ...
    belongs_to :ator_principal_midia, Ator,  foreign_key: :ator_principal
    belongs_to :ator_coadjuvante_midia, Ator, foreign_key: :ator_coadjuvante
  ...
  def changeset(midia, attrs) do
    midia
    |> cast(attrs, [:titulo, :ano_lancamento, :codigo_barras, :duracao_em_minutos, :ator_principal, :ator_coadjuvante, :genero_id, :classificacao_etaria_id, :tipo_id, :classificacao_interna_id])
    |> validate_required([:titulo, :ano_lancamento, :codigo_barras, :duracao_em_minutos, :ator_principal, :ator_coadjuvante, :genero_id, :classificacao_etaria_id, :tipo_id, :classificacao_interna_id])
    |> unique_constraint(:codigo_barras)
    |> assoc_constraint(:ator_principal_midia)
    |> assoc_constraint(:ator_coadjuvante_midia)
