...
const getItemId = (item) => {
    return item.id || item.Código_Interno || item.codigo_interno;
};
...
const columns = computed(() => {
    ...
    const priorities = [
        "id", "Código_Interno", "titulo", "nome", "sobrenome", "descricao", "sigla",
    ];
...
const delItem = async (id) => {
    ...
    try {
        ...
        if (props.deleteLabel === "Cancelar") {
            ...
            const index = itemsList.value.findIndex((i) => getItemId(i) === id);
            if (index !== -1) {
...
<template>
    ...
    <div v-else>
            <RouterLink class="btn-novo" :to="`/${props.baseRoute}/novo`">Inserir Novo</RouterLink>
                ...
                <tr v-for="item in itemsList" :key="getItemId(item)">
                    ...
                    <td class="btn-cell">
                        <template v-if="!item.Cancelada">
                            <RouterLink
                                :to="`/${props.baseRoute}/alterar/${getItemId(item)}`"
                                class="btn-alterar-form"
                            >
                                Alterar
                            </RouterLink>

                            <button @click="delItem(getItemId(item))" class="btn-excluir-form">
